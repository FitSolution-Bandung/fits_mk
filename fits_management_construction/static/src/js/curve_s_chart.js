...
import { loadJsPDF, loadHtml2Canvas, loadChartJS } from "../js/library";
...
class CurveSChart extends Component {
  ...
  async renderChart(data, projectName, lastUpdatedDate) {
    ...
    const lastUpdatedDateObj = new Date(lastUpdatedDate); // Misalnya "2024-12-10"
    console.log("Last Updated Date Object:", lastUpdatedDateObj); // Debug

    // Debugging data.dates
    ...
    const filteredDates = data.dates.filter((date) => {
      ...
      const isValid = dateObj <= lastUpdatedDateObj;
      console.log(`Checking date ${date}:`, isValid);
      return isValid;
    });

    console.log("Filtered Dates:", filteredDates);
    // const filteredDates = data.dates.filter(
    //   (date) => new Date(date) <= lastUpdatedDateObj
    // );
    const filteredPlanWeights = data.plan_weights.slice(
      0,
      formattedDates.length
    );
    const filteredActualWeights = data.actual_weights.slice(
      0,
      filteredDates.length
    );

    // console.log("Filtered Dates:", filteredDates);
    // console.log("Filtered Plan Weights:", filteredPlanWeights);
    // console.log("Filtered Actual Weights:", filteredActualWeights);
    ...
    this.chart = new Chart(ctx, {
      type: "line",
      data: {
        // labels: formattedDates.slice(0, filteredDates.length),
        labels: formattedDates,
        datasets: [
          {
            label: "Bobot Rencana (%)",
            data: filteredPlanWeights,
      ...
      options: {
        ...
        interaction: {
          intersect: true, // Tooltip hanya muncul jika kursor di titik
          mode: "nearest", // Tooltip hanya muncul untuk titik data terdekat
        },
      },
    });
  }
